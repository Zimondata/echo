module Ai
  class MultiPlanAnalyzer
    SYSTEM_PROMPT = <<~PROMPT
      ...
      2. Определить ОСНОВНОЙ тип сообщения: идея, размышление, или конкретный план
      3. НЕ РАЗБИВАЙ одну мысль на несколько записей без явных оснований
      4. Создавай отдельные записи ТОЛЬКО если в тексте четко разделены разные темы
      ...
      ВАЖНО: 
      - Если это ОДНА идея/мысль - создавай ОДНУ запись типа "idea"
      - Если это размышления - создавай ОДНУ запись типа "diary"
      - Создавай несколько записей ТОЛЬКО если пользователь явно говорит о разных вещах
      - НЕ превращай идеи в планы без прямого указания пользователя
        ...
        "entries": [
          {
            ...
            "priority": 0-10,
            "create_calendar_event": true|false (true ТОЛЬКО для типа "plan" с конкретным временем),
      ...
      Примеры КОНСЕРВАТИВНОГО анализа:
      - "Нужно допилить функцию в нашем эхо, чтобы она была лучше"
        → 1 запись idea (НЕ план, потому что нет конкретного времени/действия)
      - "Хочу прикрутить к проекту еще бизнес API Телеги"
        → 1 запись idea (это идея, а не готовый план)
      - "Завтра в 10:00 встреча с клиентом"
        → 1 план (четкое время и действие)
      - "Сегодня хороший день, много работал"
        → 1 запись diary
      - "выпил протеин на воде в обед"
        → 1 запись nutrition
      - "Утром в 9 встреча с клиентом, потом в обед купить продукты, вечером в 19:00 тренировка" 
        → 3 отдельных плана (ТОЛЬКО если четко разные действия с разным временем)

      Всегда анализируй время относительно текущей даты и времени!
      
      КРИТИЧЕСКИ ВАЖНОЕ ПРАВИЛО ДЛЯ КОНСЕРВАТИВНОГО АНАЛИЗА:
      - НЕ создавай планы для простых идей или размышлений
      - Фразы "нужно бы", "было бы хорошо", "можно сделать" = это ИДЕИ, а не планы
      - План создается ТОЛЬКО если есть четкая формулировка действия с временем
      - Если сомневаешься - лучше сделай "idea" или "diary"
      ...
      КРИТИЧЕСКИ ВАЖНО - КАЛЕНДАРНЫЕ СОБЫТИЯ:
      ⚠️ create_calendar_event = true ТОЛЬКО для планов с конкретным временем!
      ⚠️ НЕ создавай события для идей, размышлений или планов без времени!
