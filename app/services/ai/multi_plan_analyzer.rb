module Ai
  class MultiPlanAnalyzer
    SYSTEM_PROMPT = <<~PROMPT
      ...
      ⚠️ ОБЯЗАТЕЛЬНО используй СЕГОДНЯШНЮЮ дату (2025-10-15) если не указано иначе!
      ⚠️ НИКОГДА НЕ ИСПОЛЬЗУЙ ВЧЕРАШНЮЮ ДАТУ (2025-10-14)!
      ⚠️ "добавь", "планирую", "сегодня", "на сегодня" = ВСЕГДА 2025-10-15!
      
      ПРАВИЛА ДАТ:
      - "сегодня", "добавь", "планирую" = 2025-10-15 (СЕГОДНЯ)
      - "завтра" = 2025-10-16
      - Без указания даты = 2025-10-15 (СЕГОДНЯ)
      - НИКОГДА не используй 2025-10-14 (вчера)
      
      ПРАВИЛА ВРЕМЕНИ (КОНКРЕТНЫЕ ПРИМЕРЫ):
      - "17:30" → "2025-10-15T17:30:00+02:00" (НЕ 15:30!)
      - "5:30 вечера" → "2025-10-15T17:30:00+02:00"
      - "17.30" → "2025-10-15T17:30:00+02:00" 
      - "утром" → "2025-10-15T09:00:00+02:00"
      - "днем" → "2025-10-15T13:00:00+02:00"
      - "вечером" → "2025-10-15T18:00:00+02:00"
      
      ИСПРАВЛЕНИЯ:
      - "17:30!!!", "нет, в 17:30" = type: "plan_update"
      - В metadata добавь: {"correction": true, "corrected_time": "17:30"}
      
      ПЛАНЫ БЕЗ ВРЕМЕНИ (КОНКРЕТНЫЕ ПРИМЕРЫ):
      - Вход: "дневной сон"
        Выход: {
          "event_time": "2025-10-15T00:00:00+02:00",
    ...
    class << self
      ...
      def build_user_prompt(text, context, recent_events_context)
        <<~PROMPT
          ...
          🚨 КРИТИЧЕСКИ ВАЖНО:
          - Используй ТОЛЬКО СЕГОДНЯШНЮЮ дату: 2025-10-15
          - НИКОГДА НЕ используй 2025-10-14 (вчера)
